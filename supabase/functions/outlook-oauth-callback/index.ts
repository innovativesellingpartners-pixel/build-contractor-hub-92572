...
serve(async (req) => {
  try {
    ...
    if (stateData.type === 'calendar') {
      ...
      return Response.redirect(`${APP_URL}/dashboard?oauth_success=calendar&provider=outlook`);
    } else {
      await supabase
      ...
      return Response.redirect(`${APP_URL}/dashboard?oauth_success=email&provider=outlook`);
    }
  } catch (error) {
