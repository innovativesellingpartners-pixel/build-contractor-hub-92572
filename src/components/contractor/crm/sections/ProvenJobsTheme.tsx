...
export function TabNav({ tabs, activeTab, onTabChange }: TabNavProps) {
  return (
    <div className="flex overflow-x-auto bg-card border-b border-border px-1 gap-0.5 scrollbar-hide" style={{ WebkitOverflowScrolling: 'touch' }}>
      {tabs.map((tab) => (
        <button
          key={tab.id}
          onClick={() => onTabChange(tab.id)}
          className={cn(
            'flex flex-col sm:flex-row items-center gap-0.5 sm:gap-1 px-2.5 py-2 text-[10px] sm:text-xs font-medium whitespace-nowrap transition-colors border-b-2 flex-shrink-0 min-w-[3.5rem]',
            activeTab === tab.id
